...
class AopCertification
{
    ...
    public static function verifyCert($prev, $rootCerts)
    {
        ...
        for ($i = 0; $i < count($rootCerts); $i++) {
            $subjectDN = AopCertClient::array2string($rootCerts[$i]['subject']);
            $subjectMap[$subjectDN] = $rootCerts[$i];
        }
        $issuerDN = AopCertClient::array2string(($prev['issuer']));
    ...
    public static function sortByDn(&$certs)
    {
        ...
        for ($i = 0; $i < count($certs); $i++) {
            ...
            $subjectDN = AopCertClient::array2string($certs[$i]['subject']);
            $issuerDN = AopCertClient::array2string(($certs[$i]['issuer']));
    ...
    {
        $subjectDN = AopCertClient::array2string($cert['subject']);
        $issuerDN = AopCertClient::array2string($cert['issuer']);
    ...
    public static function addressingUp($subjectMap, &$certChain, $current)
    {
        ...
        $issuerDN = AopCertClient::array2string($current['issuer']);
    ...
    {
        $subjectDN = AopCertClient::array2string($current['subject']);
