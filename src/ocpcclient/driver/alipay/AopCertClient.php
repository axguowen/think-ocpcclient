...
class AopCertClient
{
    ...
    public function getCertSN($certPath)
    {
        ...
        $SN = md5(AopCertClient::array2string(array_reverse($ssl['issuer'])) . $ssl['serialNumber']);
        return $SN;
    }
    /**
     * 从证书内容中提取序列号
    ...
    public function getRootCertSN($certPath)
    {
        ...
        for ($i = 0; $i < count($array) - 1; $i++) {
            ...
            if ($ssl[$i]['signatureTypeLN'] == "sha1WithRSAEncryption" || $ssl[$i]['signatureTypeLN'] == "sha256WithRSAEncryption") {
                if ($SN == null) {
                    $SN = md5(AopCertClient::array2string(array_reverse($ssl[$i]['issuer'])) . $ssl[$i]['serialNumber']);
                } else {

                    $SN = $SN . "_" . md5(AopCertClient::array2string(array_reverse($ssl[$i]['issuer'])) . $ssl[$i]['serialNumber']);
                }
            }
        }
        return $SN;
